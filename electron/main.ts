...
import { testConnection, generateReport, generateTemplate, generateInsight, chat, type GenerateReportInput } from './ai'
...
// ==================== AI 对话 ====================
ipcMain.handle('ai:chat', async (_e, input: { messages: Array<{ role: 'system' | 'user' | 'assistant'; content: string }>; sessionId: string }) => {
  return new Promise((resolve, reject) => {
    chat(
      input.messages,
      (chunk) => emit('ai:chat-stream-chunk', { sessionId: input.sessionId, chunk }),
      (full) => { emit('ai:chat-status-changed', { sessionId: input.sessionId, status: 'completed', content: full }); resolve({ ok: true }) },
      (err) => { emit('ai:chat-status-changed', { sessionId: input.sessionId, status: 'failed', error: err.message }); reject(err) }
    )
  })
})
...
ipcMain.handle('localApi:start', async (_e, input: { port?: number }) => {
  const settings = getSettings()
